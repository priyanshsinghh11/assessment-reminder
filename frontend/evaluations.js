 ...
 *
 * A hiring manager is not shown scores anywhere -- the reason is in
 * shortlist.py's header and in MANAGER_SUBMISSION_FIELDS: a "78" beside a name
 * decides the interview before they have read a word of the work. Their column
 * therefore says nothing rather than "—", which would read as a grading
 * failure and send them asking why half the candidates are unmarked.
 *
 * `grading_incomplete` still reaches them, and still shows. It is the one fact
 * about the marking they need: it is why somebody sits where they sit. */
...
const selectedRejected = () =>
  state.rejected.filter((c) => !excluded.has(c.candidate_email));

function renderRejected() {
  const all = state.rejected || [];
    ...
    return `
      <tr class="${off ? 'row-excluded' : ''}">
        <td class="shrink"><input type="checkbox" data-email="${esc(c.candidate_email)}"
              ${off ? '' : 'checked'}></td>
        <td><div class="cand-name">${esc(c.candidate_name || '—')}</div></td>
  ...
  for (const box of $('rejectedBody').querySelectorAll('input[type=checkbox]')) {
    box.addEventListener('change', () => {
      if (box.checked) excluded.delete(box.dataset.email);
      else excluded.add(box.dataset.email);
      renderRejected();
    });
  }
...
function renderTop() {
  ...
  // The score is not on the review payload and deliberately so: that surface
  // is the manager's emailed link, which never sees one. On this page the
  // reader is signed in and the Candidates tab is showing them scores one tab
  // away, so it is read back out of what this page already holds -- and simply
  // left blank until that request lands.
  const scoreOf = (id) =>
    state.candidates.find((c) => c.id === id)?.evaluation?.score;
  ...
  $('topBody').innerHTML = rows.map((c) => {
    ...
    const score = scoreOf(c.submission_id);
    ...
    return `
      <tr class="top-row row-click${picked ? ' is-picked' : ''}${
        ...
        <td class="num"><span class="score-cell ${scoreClass(score)}">${
          fmtScore(score)}</span></td>
