...
AUTOMATION IS PAUSED. A plain `python manage.py reminder` will not scan or send while
...
Usage:
    python manage.py reminder                # normal run (blocked while paused)
    python manage.py reminder --force        # send anyway, paused or not
    python manage.py reminder --preview      # print the emails here, send nothing
    python manage.py reminder --dry-run      # log what WOULD happen, send nothing
    python manage.py reminder --scan-only    # just list who qualifies
    python manage.py reminder --job CODE     # restrict to one job shortcode
    python manage.py reminder --limit N      # send at most N emails this run
    python manage.py reminder --help         # every flag, spelled exactly
...
from backend.mail.brevo_client import send_reminder_email, build_reminder_email
from backend.mail import unsubscribe
from backend.utils import state_key
...
from backend.db.reminder_log import ReminderLogUnavailable
...
# ---------------------------------------------------------------------------
#
# setup_logging() now lives in backend/logging_setup.py -- see the note
# at the top of that module for why it moved. It is re-exported here because
# `from backend.mail.reminder import setup_logging` is what wsgi.py,
# the dashboard and every pipeline CLI have always written, and because this
# module's own main() calls it.

from backend.logging_setup import setup_logging  # noqa: E402,F401
