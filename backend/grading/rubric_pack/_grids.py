"""
The 19 scoring grids, as data. Nothing here executes.

SPLIT OUT OF rubric_pack.py, NOT CONVERTED. This is the same tuple it always
was, comments and all, in its own file because it is 7,400 lines and it was
sitting on top of the 450 lines of logic that read it -- which meant the
functions every caller actually uses began at line 7,696 of an 8,134-line
module. Moving the data left the logic legible; changing its format would have
cost more than the file size ever did:

  * The 360 comment lines interleaved below are the rationale for individual
    anchors and weights, and JSON cannot hold them.
  * The anchors quote real task content across implicit string concatenation,
    which is readable here and one very long line per anchor in JSON.
  * Editing a pack grid in reviewed source is the deliberate gate on
    hand-authored rubrics -- see the refusal in server.api_derive_rubric(),
    which tells you to "edit rubric_pack.py to move that bar". Grids we
    generate rather than author already live in assessments/grid-<slug>.json,
    and that difference in format is the difference in provenance.

`_validate_pack()` in __init__.py still runs over every grid below at import,
so a grid that does not add to 100 still stops the process rather than
silently rescaling a family.
"""
