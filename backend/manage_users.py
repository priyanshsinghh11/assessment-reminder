...
work. This exists for the two moments that screen cannot cover:
  ...
  * the locked-out admin, when the only way back in is the machine itself.
    ...
    python manage.py users add priya@ajaia.ai --admin
    python manage.py users add sam@ajaia.ai --name "Sam Okafor"
    python manage.py users passwd sam@ajaia.ai
    python manage.py users promote sam@ajaia.ai
    python manage.py users demote sam@ajaia.ai
    python manage.py users disable sam@ajaia.ai
    python manage.py users remove sam@ajaia.ai
    python manage.py users roles sam@ajaia.ai
...
from backend.config import AUTH_ENABLED, PASSWORD_MIN_LENGTH
...
def cmd_list(args) -> int:
    ...
    if not users:
        print("No accounts yet. Create one:  python manage.py users add "
              "--admin <email>")
        return 0
...
def cmd_disable(args) -> int:
    ...
    print(f"{args.email} is disabled and signed out. Re-enable with: "
          f"python manage.py users enable {args.email}")
    return 0
