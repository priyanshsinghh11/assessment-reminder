...
point. backend/core/__init__.py states the rule -- "this package imports from
...
imported backend.database.reminder_log, so `core`, the bottom of the stack,
...
backend/database/reminder_state.py. state_key() stayed behind because both
...
def aware(value) -> Optional[datetime]:
    ...
    THREE COPIES OF THIS EXISTED -- in accounts/auth.py, database/mongo_store.py
    and database/reminder_log.py -- and they did not agree. auth's and
    reminder_log's returned None for a value they could not use; mongo_store's
