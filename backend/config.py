...
# The repository root. Every path in the project is resolved from here rather
...
#
# WRITTEN AS "the directory holding backend/" RATHER THAN A parents[n] COUNT.
# It used to be a count, and moving this module up one level (it was
# backend/core/config.py) left the count pointing one directory above the
# repository. Nothing raised: .env silently stopped loading, FRONTEND_DIR
# pointed at a directory that does not exist, and the dashboard answered 404
# for its own stylesheet. Derived from the package directory, the expression
# does not care how deep this file sits inside backend/.
PROJECT_ROOT = Path(__file__).resolve().parent.parent

# And checked, because the failure above is invisible: the app boots, every
# route registers, and it serves nothing. Cheaper to refuse to start.
if not (PROJECT_ROOT / "backend" / "config.py").is_file():
    raise RuntimeError(
        f"PROJECT_ROOT resolved to {PROJECT_ROOT}, which does not contain "
        "backend/config.py. Every path in this project is resolved from it, "
        "so nothing below would work. This module has to live inside the "
        "backend package, in a checkout with the repository layout intact.")
...
# the substitution happens on the way out of `store.get_role(s)`, and the
...
# evaluation. `store.upsert_workable_candidates` keys on the Workable
...
# unattended `python manage.py reminder` refuses to send. Every scrape and every send
